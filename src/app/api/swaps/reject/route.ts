...
export const POST = requireAuth(async (user, request: NextRequest) => {
  try {
    ...
    const { swapRequestId } = RejectSwapSchema.parse(body);
    ...
    // Check if user is the assigned person (can reject the swap)
    if (swapRequest.assignment.userId !== user.id) {
      return NextResponse.json(
        { success: false, error: 'You can only reject swaps for your own assignments' },
        { status: 403 }
      );
    }
